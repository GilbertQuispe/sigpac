...
'use client'
import { useState, useActionState, useEffect } from 'react' // quita useTransition, agrega useActionState, useEffect
import Link from 'next/link'
import { useRouter } from 'next/navigation' // NUEVO
import { checkDni, register } from '@/app/actions/auth'
/* import { checkDniWithAuth, register } from '@/app/actions/auth' // CAMBIO 1 */
...
const [state, formAction, isPending] = useActionState(register, null) // NUEVO

// NUEVO: Para leer el resultado
useEffect(() => {
  if (state?.error) setError(state.error)
  if (state?.success) router.push(`/?success=${encodeURIComponent(state.success)}`)
}, [state, router])
...
/* if (result.error) { 
      ...
      if(result.error === 'Usted ya es miembro') setEsMiembro(true) // CAMBIO 5
    } else { 
 ...
 /*  const handleSubmit = async (formData: FormData) => {
  ...
  return (
    <form action={formAction} className="w-full max-w-sm mx-auto space-y-6">
      ...
      <fieldset className="relative border rounded-lg border-[var(--color-blue-2)] focus-within:border-[var(--color-blue-1)] transition">
        ...
        <div className="flex items-center h-12">
          <input 
            ...
            value={dni}
            onChange={e => { setDni(e.target.value.replace(/\D/g, '')); setPersona(null); setError(''); }}
      ...
      <div className="flex gap-4 pt-2">
        ...
        <button type="submit" disabled={isLocked || isPending} className="w-full bg-[var(--color-blue-1)] hover:bg-[var(--color-dark-2)] text-white font-semibold py-3.5 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed">
